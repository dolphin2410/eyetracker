...
def application_callback(image: EyetrackerImage, camera_context: CameraContext):
    ...
    x_w = (x_pos_mask * white_part).sum() / white_part.sum()
    y_w = (y_pos_mask * white_part).sum() / white_part.sum()
    x_b = (x_pos_mask * black_part).sum() / black_part.sum()
    y_b = (y_pos_mask * black_part).sum() / black_part.sum()

    if "start_record" in camera_context.settings:
        if camera_context.settings['start_record']:
            print(x_w, y_w, x_b, y_b)
            camera_context.eyetracker_history.append_frame(EyetrackerHistoryFrame(x_w, y_w, x_b, y_b))

    camera_context.eyetracker_history.curr_frame = EyetrackerHistoryFrame(x_w, y_w, x_b, y_b)
